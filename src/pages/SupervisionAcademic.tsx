...
import { AcademicSupervision, SupervisionStatus, SupervisionInstrument, SupervisionIndicator } from "../types";

export default function SupervisionAcademic() {
  const { user } = useAuth();
  ...
  const isGuru = userRoles.includes("guru") || user?.role === "guru";
  ...
  const hasAccess = isAdmin || isKepalaSekolah || isWakilKepalaSekolah || isKetuaYayasan || isGuru;
  ...
  const supervisionsFilter = useMemo(() => {
    ...
    const isStrictTeacher = isGuru && !isAdmin && !isKepalaSekolah && !isWakilKepalaSekolah;
    if (isStrictTeacher) {
      f.teacherId = user?.teacherId || "NO_TEACHER_LINK";
    }
    return f;
  }, [isGuru, isAdmin, isKepalaSekolah, isWakilKepalaSekolah, user, filterYear, filterSem, filterSupervisorId, filterStatus]);
  ...
  const {
    ...
    isUpdating,
    deleteSupervision
  } = useAcademicSupervisions(supervisionsFilter);
  ...
  // Load all Academic instruments
  const { instruments = [] } = useSupervisionInstruments({ type: "Akademik" });
  ...
  const [selectedSupervision, setSelectedSupervision] = useState<AcademicSupervision | null>(null);
  ...
  // Load supervision result (to display in detail modal or edit in assessment modal)
  ...
  const handleOpenEdit = (sup: AcademicSupervision) => {
    if (isReadOnly) return;
  ...
  const handleOpenDetail = (sup: AcademicSupervision) => {
    setSelectedSupervision(sup);
    ...
    setResultTargetId(sup.id); // Triggers loading the result for this supervision
  ...
  const handleOpenDelete = (sup: AcademicSupervision) => {
    if (isReadOnly) return;
    setSelectedSupervision(sup);
    setIsDeleteOpen(true);
  };

  // Open assessment modal and fetch existing results if any
  const handleOpenAssessment = (sup: AcademicSupervision) => {
    setSelectedSupervision(sup);
    setResultTargetId(sup.id); // Triggers load of existing result
  ...
  // If existing result is fetched, populate the assessment form
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    if (!formData.teacherId) errors.teacherId = "Guru yang disupervisi wajib dipilih.";
  ...
  if (isGlobalLoading) {
    return <Loading label="Memuat modul supervisi akademik..." />;
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
        ...
        <p className="text-sm text-slate-500 dark:text-zinc-400 mt-2 max-w-md leading-relaxed">
          Halaman ini khusus untuk Guru, Kepala Sekolah, Wakil Kepala Sekolah, dan Yayasan. Akun Anda tidak memiliki hak akses untuk membuka halaman ini.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-zinc-800 pb-5">
        <div>
          ...
          <p className="text-slate-500 dark:text-zinc-400 text-sm mt-1">
            Penjadwalan, pemantauan, dan penilaian hasil supervisi pembelajaran Guru di kelas berdasarkan instrumen terstandar.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-xs flex flex-col md:flex-row gap-3">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Cari guru yang disupervisi atau supervisor..."
            value={searchQuery}
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-xs overflow-hidden">
        {filteredSupervisions.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-md font-bold text-slate-800 dark:text-zinc-200">Belum ada data supervisi akademik</h3>
            <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
              Silakan jadwalkan atau tambahkan supervisi akademik menggunakan tombol di kanan atas.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-zinc-950 border-b border-slate-200 dark:border-zinc-800">
                  <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Guru Disupervisi</th>
      ...
      {/* CREATE MODAL */}
      <Dialog isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Jadwalkan Supervisi Akademik" size="lg">
        <form onSubmit={handleCreateSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Guru yang Disupervisi *</label>
              <select
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Berikan rincian kelas, materi pokok, atau catatan pra-observasi..."
              className="w-full px-3 py-2 border border-slate-250 dark:border-zinc-800 rounded-xl bg-slate-50 dark:bg-zinc-950 text-sm focus:ring-2 focus:ring-blue-500/20"
            />
          </div>
        ...
        <form onSubmit={handleEditSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              value={formData.notes}
              onChange={(e) => setFormData(prev => ({ ...prev, notes: e.target.value }))}
              placeholder="Berikan catatan kualitatif hasil observasi kelas..."
              className="w-full px-3 py-2 border border-slate-250 dark:border-zinc-800 rounded-xl bg-slate-50 dark:bg-zinc-950 text-sm focus:ring-2 focus:ring-blue-500/20"
            />
          </div>
      ...
      {/* ASSESSMENT INPUT FORM DIALOG (SUPERVISI GURU) */}
      <Dialog
        isOpen={isAssessmentOpen}
        onClose={() => setIsAssessmentOpen(false)}
        title={`Lembar Penilaian Akademik - ${selectedSupervision?.teacherName}`}
        size="lg"
      >
      ...
      {/* DETAIL MODAL */}
      <Dialog isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} title="Detail Kegiatan & Hasil Supervisi" size="lg">
        {selectedSupervision && (
          <div className="space-y-5">
            <div className="bg-slate-50 dark:bg-zinc-950 p-4 rounded-xl border border-slate-200 dark:border-zinc-800 space-y-2.5">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">GURU DISUPERVISI</span>
            ...
            <div>
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1">CATATAN & REKOMENDASI HASIL OBSERVASI</span>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 bg-rose-50 dark:bg-rose-950/30 text-rose-700 dark:text-rose-300 rounded-xl border border-rose-250 dark:border-rose-900/50">
            ...
            <p className="text-xs font-semibold">
              Apakah Anda yakin ingin menghapus jadwal supervisi untuk guru <strong>{selectedSupervision?.teacherName}</strong>? Tindakan ini tidak dapat dibatalkan.
            </p>
          </div>
